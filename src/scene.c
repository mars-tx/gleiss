...
Scene* load_Scene(const char* filename){
    ...
    //Read Counts
    read_values= fscanf(file, "%d %d", &scene->mesh_count, &scene->object_count);
    if (read_values != 2){
        fclose(file);
        free(scene);
        return NULL;
    ...
    for (int m= 0; m< scene->mesh_count; m++) {
        ...
        for (int i= 0; i < v_count; i++) {
            ...
            read_values= fscanf(file, "%f %f %f", &vertices[i].pos.x, &vertices[i].pos.y, &vertices[i].pos.z);
            if (read_values != 3){
                fclose(file);
                free_Scene(scene);
                return NULL;
        ...
        for (int i= 0; i < f_count; i++) {
            ...
            int v1, v2, v3;
            read_values= fscanf(file, "%d %d %d", &v1, &v2, &v3);
            if (read_values != 3){
                fclose(file);
                free_Scene(scene);
                return NULL;
    ...
    for (int o= 0; o < scene->object_count; o++) {
        ...
        read_values= fscanf(file, "%d %f %f %f %f %f %f %f %f %f", 
